...
namespace KAWebHost.Controllers
{
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        public RoxyFilemanController(IWebHostEnvironment env)
        {
            ...
            _tempPath = _systemRootPath + "\\wwwroot\\uploads\\temp";
        ...
        {
            _settings = JsonConvert.DeserializeObject<Dictionary<string, string>>(System.IO.File.ReadAllText(_systemRootPath + "/wwwroot/assets/plugins/custom/Roxy_Fileman/conf.json"));
            string langFile = _systemRootPath + "/wwwroot/assets/plugins/custom/Roxy_Fileman/lang/" + GetSetting("LANG") + ".json";
            if (!System.IO.File.Exists(langFile)) langFile = _systemRootPath + "/wwwroot/assets/plugins/custom/Roxy_Fileman/lang/en.json";
        ...
        public IActionResult UPLOAD(string d)
        {
            try
            {
                var path = HttpContext.Request.Form["d"][0];
                d = MakePhysicalPath(path);
                CheckPath(d);
                d = FixPath(d);
